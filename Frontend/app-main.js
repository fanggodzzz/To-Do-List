...
function normalizeTodo(todo) {
  ...
  return {
    ...
    ac_description: todo.acDescription ?? todo.ac_description,
    ac_tag_id: resolveTagById(tagId),
    ac_completed: todo.acCompleted ?? todo.ac_completed,
  };
}
...
function loadTags() {
  return authorizedFetch("/tags")
    ...
    .then((data) => {
      state.tags = Array.isArray(data)
        ? data.map(normalizeTag).filter(Boolean)
        : [];
      state.todos = state.todos.map(normalizeTodo).filter(Boolean);
      buildTagOptions(addTagSelect);
      buildTagOptions(detailTag);
      renderTagLabels();
      renderCalendar();
  ...
  return authorizedFetch("/upcoming")
    ...
    .then((data) => {
      state.upcoming = Array.isArray(data)
        ? data.map(normalizeTodo).filter(Boolean)
        : [];
      renderSummaryBoxes();
    })
    .catch((error) => {
...
function loadOverdueTasks() {
  return authorizedFetch("/overdue")
    ...
    .then((data) => {
      state.overdue = Array.isArray(data)
        ? data.map(normalizeTodo).filter(Boolean)
        : [];
      renderSummaryBoxes();
    })
...
function loadTodos() {
  ...
  return authorizedFetch(`/${getCurrentMonthQuery()}`)
    ...
    .then((data) => {
      state.todos = Array.isArray(data)
        ? data.map(normalizeTodo).filter(Boolean)
        : [];
      renderCalendar();
      setStatus("");
